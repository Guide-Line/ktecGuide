			...
			$("#popup").animate({}, 5,
				function () {
					...
					$('.popup_in_img').error(function () {
						//·ÎµåµÈ ÆË¾÷ÀÌ¹ÌÁö°¡ 404 error ÀÌ¸é ½ÇÇà
						//alert("ÀÌ¹ÌÁö °æ·Î°¡ ÀÔ·ÂµÇ¾úÀ¸³ª, Àß¸øµÇ¾ú½À´Ï´Ù.");
						...
						$(".imgUrl").val("");
						popup_resize(1); //ÆË¾÷Ã¢ ¸®»çÀÌÁî Å¸ÀÔ1¹ø
						//}).attr( "src", imgUrl);
					}).load(function () {
						...
						popup_resize(2); //ÆË¾÷Ã¢ ¸®»çÀÌÁî Å¸ÀÔ2¹ø
					});
...
function menuMake() {
	var data_index = 1;
	$("#sortable .btn>a").off(); //ÀÌº¥ÁÖ ÁßÃ¸¹æÁö¸¦ À§ÇÑ ÃÊ±âÈ­
	$('#sortable').append('' +
...
//ÆË¾÷ : º¸ÀÌ±â 
function popup_open() {
	popup_resize(0) //ÆË¾÷Ã¢ ¸®»çÀÌÁî Å¸ÀÔ0¹ø
...
function popup_clear() {
	$(".imgUrl").val(""); //ÀÔ·ÂÃ¢ ÀÌ¹ÌÁö °æ·Î
	$(".linkUrl").val(""); //ÀÔ·ÂÃ¢ ¸µÅ© °æ·Î
	$(".popup_in_img").remove();
	imgUrl = ""; //ÀÔ·ÂÃ¢ ÀÌ¹ÌÁö À¯È¿½Ã ÀúÀåµÇ´Â º¯¼ö 
	$("#popup").css({
		height: "260px"
	});
	input_state = false;
...
//ÆË¾÷ - È®ÀÎ Å¬¸¯ - ÇÏ´Ü½æ³×ÀÏ »ı¼º( ui-update )
function popup_info_check() {
	...
	if (img_str == null || img_str == undefined || img_str == "") {
		//alert("ÀÌ¹ÌÁö °æ·Î¸¦ ÀÔ·ÂÇØÁÖ¼¼¿ä")
	} else {
		menuMake();
		popup_close();
		//¸¶Áö¸· µÎ¹øÂ° ¿ä¼Ò¿¡ ÀÌ¹ÌÁö »ğÀÔ
...
function top_event() {
	...
	$(".downBtn").on('click', function () {
		if (coding_souce_message == "yes") {
			...
			alert("¼Ò½ºº¸±â ¹öÆ°À» Å¬¸¯ÇÏ¿© ÄÚµå¸¦ »ı¼º or °»½Å ÇØÁÖ¼¼¿ä!")
		}
	});
...
function changeTarget(str){
	...
	if(isWeb == "desktop"){
		return _str.replace(/target="_blank"/g,'target="_blank"');//À¥ ÀÇ°á°ú°ª
	}else{
		return _str.replace(/target="_blank"/g,'target="_self"');//¸ğ¹ÙÀÏÀÇ °á°ú°ª
	}
}
