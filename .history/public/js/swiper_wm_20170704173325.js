...
var str=""; // ?��붾뵫�궡�슜�쓣 �떞�뒗 ?��몄옄�뿴
var total = -1;
var isView = -1;
var pop_img; // �뙘�뾽�뿉 �몴�떆�릺�뒗 �씠誘몄�� ����?��
var pop_link; // �뙘�뾽�뿉 �몴�떆�릺�뒗 �씠誘몄�� 留곹�? ����?��
var isBtnNum = 0;
var input_state = false;
var ptemp = -1;
var isWeb = "desktop"; //desktop or mobile
var coding_souce_message = "no"; //�냼�뒪蹂닿�? �겢?���? ��???���?
...
function allClear() {
	total = -1;
	isView = -1;
	pop_img; // �뙘�뾽�뿉 �몴�떆�릺�뒗 �씠誘몄�� ����?��
	pop_link; // �뙘�뾽�뿉 �몴�떆�릺�뒗 �씠誘몄�� 留곹�? ����?��
	isBtnNum = 0;
	input_state = false;
	ptemp = -1;
	...
	$('#sortable').empty();
	$("#p_wrapper_slide").empty();
	$('#resultArea textarea').val(""); //�냼�뒪蹂닿린李�? ?��?��린�?��
	popup_clear();
	coding_souce_message = "no";
}
			...
			$("#popup").animate({}, 5,
				function () {
					...
					$('.popup_in_img').error(function () {
						//濡쒕뱶��? �뙘�뾽�씠誘몄��媛� 404 error �씠硫� �떎�뻾
						//alert("�씠誘몄�� 寃쎈줈媛�? �엯�젰�릺��??�쑝�굹, �옒紐삳릺��?�뒿�땲�떎.");
						$('.popup_in_img').css("display", "none");

						$this.parents("#popup").find("#holder").append('' +
							'<div class="not_img_message" style="display:block;">�빐�떦 留곹겕�?�� �젙蹂�??�� ?��?��?���삱�닔 �뾾�뒿�땲�떎<br>留곹겕瑜�? �떎�떆 �솗�씤 �빐 二쇱�?�슂</div>');

						$(".imgUrl").val("");
						popup_resize(1); //�뙘�뾽李� ?��?�궗�씠利� ����?��1踰�
						//}).attr( "src", imgUrl);
					}).load(function () {

						console.log('------------------------------------');
						console.log('�젙�긽�쟻�씤 �씠誘몄�� 濡쒕�?');
						console.log('------------------------------------');
						$('.popup_in_img').css("display", "block");
						popup_resize(2); //�뙘�뾽李� ?��?�궗�씠利� ����?��2踰�
					});
...
function menuMake() {
	var data_index = 1;
	$("#sortable .btn>a").off(); //�씠踰ㅼ�? 以묒꺽諛?����?���? �쐞�븳 ?��?��린�?��
	$('#sortable').append('' +
...
function popup_clear() {
	$(".imgUrl").val(""); //�엯�젰李� �씠誘몄�� 寃쎈�?
	$(".linkUrl").val(""); //�엯�젰李� 留곹�? 寃쎈�?
	$(".popup_in_img").remove();
	imgUrl = ""; //�엯�젰李� �씠誘몄�� ��??�슚�떆 ����?���릺�뒗 蹂��닔 
	$("#popup").css({
		height: "260px"
	});
	input_state = false;
...
//�뙘�뾽 - �솗�씤 �겢?���? - �븯�떒�뜽�꽕�씪 �깮�꽦( ui-update )
function popup_info_check() {
	if (input_state == false) {
		alert("�씠誘몄�� 寃쎈줈瑜�? �엯�젰�빐二쇱�?�슂")
		return;
	}
...
function top_event() {
	...
	$(".downBtn").on('click', function () {
		if (coding_souce_message == "yes") {
			...
			alert("�냼�뒪蹂닿�? 踰꾪?���쓣 �겢?�����?�뿬 ?��붾뱶?���? �깮�꽦 or 媛깆?�� �빐二쇱�?�슂!")
		}
	});
...
function changeTarget(str){
	...
	if(isWeb == "desktop"){
		...
		return _str.replace(/target="_blank"/g,'target="_self"');//紐⑤�?�씪
	}
}
